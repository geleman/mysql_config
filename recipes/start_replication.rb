...
if node['mysql_config']['instance_name'] == 'slave'
  bash 'start replication' do
    user 'mysql'
    code <<-EOF
    mysql -uroot -p#{slvopts['password']} -S #{node['mysql_config']['socket']} -e "change master to master_host = '10.84.101.100',
    ...
    start slave;"
    EOF
    not_if "mysql -uroot -p#{slvopts['password']} -S #{node['mysql_config']['socket']} -e
